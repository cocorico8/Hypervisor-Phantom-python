...
def _acquire_tkg_source():
    ...
    try:
        utils.run_with_spinner(
            ["git", "clone", "--depth=1", TKG_URL, str(tkg_path)],
            cwd=Path.cwd()
        )
...
def _build_and_install(distro: str):
    ...
    tkg_path = SRC_DIR / TKG_DIR_NAME
    utils.info("Starting kernel build and installation process... This will take a very long time.")
    
    try:
        if distro == "Arch":
            original_user = os.environ.get("SUDO_USER", getpass.getuser())
            if original_user == "root":
                utils.fail("For Arch, this script must be run via sudo from a normal user account to build packages safely.")
            
            utils.warn(f"Running 'makepkg' as user '{original_user}'. This is a required security measure.")
...
def _create_systemd_boot_entry():
    ...
    try:
        root_dev = subprocess.check_output(["findmnt", "-no", "SOURCE", "/"], text=True).strip()
        partuuid = subprocess.check_output(["blkid", "-s", "PARTUUID", "-o", "value", root_dev], text=True).strip()
        if not partuuid:
...
def main(distro: str, cpu_vendor: str):
    ...
    if utils.yes_or_no("Configuration is complete. Proceed with kernel build and installation?"):
        _build_and_install(distro)
        if utils.yes_or_no("Kernel installed. Would you like to create a systemd-boot entry for it?"):
