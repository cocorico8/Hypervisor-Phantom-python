...
def _get_cpu_topology():
    ...
    try:
        ...
        utils.info(
            f"Detected: {cores} cores * {threads} threads = {total_vcpus} total vCPUs."
        )
...
def main(cpu_vendor: str):
    ...
    template_file = (
        template_dir
        / f"template-{'intel' if 'GenuineIntel' in cpu_vendor else 'amd'}.xml"
    )
    ...
    try:
        ...
        utils.run_with_spinner(
            ["sudo", "virsh", "define", temp_xml_path], cwd=Path.cwd()
        )
    ...
    try:
        ...
        utils.error(
            "Failed to inject SMBIOS data. This is an optional step and can be ignored."
        )
    ...
    try:
        ...
        utils.error(
            "Failed to set MAC address. This can be set manually in virt-manager."
        )

    utils.info(f"VM '{vm_name}' has been created successfully!")
    utils.warn(
        "Further customization (e.g., adding disks, USB devices) should be done via virt-manager or by manually editing the XML."
    )
