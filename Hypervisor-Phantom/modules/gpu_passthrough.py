...
def _detect_bootloader() -> tuple[str, Path | None]:
    ...
    sd_boot_dirs = [Path("/boot/loader/entries"), Path("/boot/efi/loader/entries"), Path("/efi/loader/entries")]
...
def _select_gpu_and_validate_iommu() -> tuple[str, str, str]:
    ...
    if len(gpus) == 1:
        utils.warn("Only one GPU detected. Passing it through will likely leave the host without a display.")
    ...
    if not iommu_group_path.is_symlink():
        utils.fail("Selected GPU is not in an IOMMU group. Enable IOMMU in your BIOS/UEFI.")
    ...
    for device in group_devices_path.iterdir():
        ...
        if not device.name.startswith(pci_bus_dev):
            bad_functions.append(f"  - {device.name} ({_run_cmd(['lspci', '-s', device.name])})")

    if bad_functions:
        utils.fail(f"Bad IOMMU group detected!\nGroup #{group_id} also contains essential non-GPU devices:\n" +
                   "\n".join(bad_functions) +
...
def _create_vfio_conf(hw_ids: str, vendor_id: str):
    ...
    for driver in drivers:
        if driver: lines.append(f"softdep {driver} pre: vfio-pci")
    utils.update_config_file(str(VFIO_CONF_PATH), ".*", "\n".join(lines), append_if_missing=True)
    utils.log(f"Created/updated {VFIO_CONF_PATH}")


def _get_updated_opts_string(current_opts: str, new_opts_str: str, is_revert: bool) -> str:
    """
...
def _modify_bootloader(bootloader_type: str, config_path: Path, new_opts_str: str, is_revert: bool):
            ...
            try:
                ...
                for i, line in enumerate(lines):
                    if line.strip().startswith("options"):
                        ...
                        break
                utils.update_config_file(str(conf_file), ".+", "\n".join(lines), append_if_missing=False)
            except Exception as e:
        ...
        if shutil.which(grub_cmd):
            ...
            if grub_cmd != "update-grub":
                if Path("/boot/grub/grub.cfg").exists():
                    ...
                    utils.fail("Cannot find grub.cfg path."); return
            cmd = ["sudo", grub_cmd]
            if output_path: cmd.extend(["-o", output_path])
            try:
...
def main(vendor_id: str):
    ...
    if changes_made and boot_type == "GRUB":
        if utils.yes_or_no("Bootloader configuration was changed. Rebuild GRUB config now?"):
            _rebuild_grub()
