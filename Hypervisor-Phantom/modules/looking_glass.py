...
REQUIRED_PACKAGES = {
    "Arch": [
        "cmake", "gcc", "libgl", "fontconfig", "spice-protocol", "make", "nettle",
        "pkgconf", "libxi", "libxinerama", "wayland-protocols", "ttf-dejavu", "libsamplerate", "curl"
    ],
    "Debian": [
        "binutils-dev", "cmake", "fonts-dejavu-core", "libfontconfig-dev",
        "gcc", "g++", "pkg-config", "libegl-dev", "libgl-dev", "libspice-protocol-dev",
        "nettle-dev", "libx11-dev", "libwayland-dev", "wayland-protocols",
        "libpipewire-0.3-dev", "libsamplerate0-dev", "curl"
    ],
    "openSUSE": [
        "binutils-devel", "clang", "cmake", "dejavu-fonts", "fontconfig-devel", "gcc", "gcc-c++",
        "glibc-locale", "libdecor-devel", "libglvnd-devel", "libnettle-devel", "libsamplerate-devel",
        "libSDL2-2_0-0", "libSDL2_ttf-2_0-0", "libvulkan1", "libwayland-egl1", "libxkbcommon-devel",
        "libXpresent-devel", "libXrandr-devel", "libXss-devel", "make", "Mesa-libGLESv3-devel",
        "pipewire-devel", "pkgconf-pkg-config", "pkgconf", "spice-protocol-devel", "vulkan-devel",
        "wayland-devel", "zlib-devel-static", "libXi-devel", "libXinerama-devel", "libXcursor-devel",
        "dkms", "Mesa-libGL-devel", "Mesa-libGLESv2-devel", "libzstd-devel-static",
        "libconfig++-devel", "SDL2-devel", "curl"
    ],
    "Fedora": [
        "cmake", "gcc", "gcc-c++", "libglvnd-devel", "fontconfig-devel", "spice-protocol",
        "make", "nettle-devel", "pkgconf-pkg-config", "binutils-devel", "libXi-devel",
        "libXinerama-devel", "libXcursor-devel", "libXpresent-devel", "libxkbcommon-x11-devel",
        "wayland-devel", "wayland-protocols-devel", "libXScrnSaver-devel", "libXrandr-devel",
        "dejavu-sans-mono-fonts", "libdecor-devel", "pipewire-devel", "libsamplerate-devel",
        "dkms", "kernel-devel", "kernel-headers", "curl"
    ]
}
...
def _configure_ivshmem_shmem():
    ...
    utils.update_config_file(str(conf_path), ".*", conf_content) # Replaces or creates the file
    ...
    utils.info("Configuration successful. To easily run the client, you can add this alias to your shell's config file (e.g., ~/.bashrc):")
    alias_command = "alias lg='/usr/local/bin/looking-glass-client -S -K -1'"
    print(f"\n  {utils.Fore.CYAN}{alias_command}\n")
    utils.warn("TIP: After adding the alias, run 'source ~/.bashrc' or open a new terminal, then simply type 'lg' to start.")
...
def _configure_ivshmem_kvmfr():
    ...
    try:
        ...
        utils.run_with_spinner(["sudo", "modprobe", "kvmfr", f"static_size_mb={memory_size_mb}"], Path.cwd())
        
        # Make it persistent
        (Path("/etc/modprobe.d/kvmfr.conf")).write_text(f"options kvmfr static_size_mb={memory_size_mb}\n")
        utils.log("Created modprobe options file.")
        
        (Path("/etc/modules-load.d/kvmfr.conf")).write_text("# KVMFR Looking Glass module\nkvmfr\n")
        ...
        if kvmfr_dev.exists():
            ...
            utils.warn(f"Device {kvmfr_dev} not found. Permissions not set. It may be created on next boot.")
...
def main(distro: str, cpu_vendor: str):
    ...
    if distro not in REQUIRED_PACKAGES:
        utils.fail(f"Looking Glass setup is not supported for the detected distro: {distro}")
    ...
    if utils.yes_or_no("Configure IVSHMEM using the standard shared memory file method?"):
        _configure_ivshmem_shmem()

    if utils.yes_or_no("Configure the KVMFR kernel module for high-performance capture? (Recommended)"):
